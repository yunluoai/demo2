...
function initLobbyModule()
	...
	local id, data_str = requestPlayerDBData("hengheng")
	print(id, data_str)
	Utils.saveAccount("hengheng", 114514)
	Utils.savePlayerDBDataNow(114514, "1919810 just a new data")
end
...
function handleLogin(msg)
    ...
    if data ~= nil and data["pwd"] ~= nil and data["pwd"] == msg["pwd"] then
        print("login ok")
        local resp = {}
...
function handleSendPlayerData(msg)
	...
	local resp = {};
	resp["type"] = MsgDef.ACK_TYPE.SYNC_SUCCEED
...
function update()
    ...
    elseif (msg["type"] == MsgDef.REQ_TYPE.REGISTER) then -- 注册
        handleRegister(msg)
	elseif (msg["type"] == MsgDef.REQ_TYPE.SEND_PLAYER_DATA) then -- 发送用户数据
		handleSendPlayerData(msg)
	elseif (msg["type"] == MsgDef.REQ_TYPE.REC_PLAYER_DATA) then -- 接受用户数据
	...
	elseif (msg["type"] == MsgDef.REQ_TYPE.SETUP_CONNECTION) then
